...
4. For each `### Migration`, run its verification first. If the target state is
   ...
   the change.
5. For incomplete work, create or resume `.state/migrations/v<version>.yml`
   with `status`, `current_step`, and `last_note`. Explain the intended local
   changes and ask for confirmation before changing files.
...
8. Mark the migration journal `complete`, then update `installed_version` in
   `brainiac_me.md` only after every migration and validation succeeds.

The `installed_version` is the final completion marker. Migration journals
record resumable progress; they do not replace validation. Migrations must be
...
`installed_version` unchanged, keep backups and the incomplete journal,
explain the failed step, and restart the ordered checks safely on the next
session.
...
- If a version has no migration, record `none` in its changelog entry; do not
  create a journal.
